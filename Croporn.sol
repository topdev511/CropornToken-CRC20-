...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    address public constant WCRO = address(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca); //WCRO
    // for testnet: 0x4fea4C520f9c4a02993d64994280C24790444576
    // for mainnet: 0x5C7F8A570d578ED84E63fdFA7b1eE72dEae1AE23
    ...
    function distributeWCRODividends(uint256 amount) public onlyOwner{
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        bool success = IERC20(WCRO).transfer(user, _withdrawableDividend);
...
/* CROPORN.sol */
contract CROPORN is ERC20, Ownable {
    using SafeMath for uint256;
    using Address for address payable;

    CropornDividendTracker public dividendTracker;
    ...
    uint256 public WCRORewardsFee = 5;
    ...
    uint256 public totalFees = liquidityFee.add(WCRORewardsFee).add(marketingFee).add(burnFee).add(buybackFee).add(algorithmFee);
    
    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public _marketingWalletAddress = 0x9a4783c4FbD41Ba7229Db4AEa806577ffE579763;
    address public _algorithmWalletAddress = 0x0eca52fcfc926B6Bf582A3a63347A5C88CD83802;
    address public _buybackWalletAddress = 0x9c8cBa4C95544BEF176Fa1cD7FA5c2014960f29D;
    ...
    event WCRORewardsFeeUpdated(address indexed operator, uint256 oldWCRORewardsfee, uint256 newWCRORewardsfee);
    ...
    event CropornTokensWithdrawn(address indexed operator, address indexed recipient, uint256 amount);
    ...
    constructor() public ERC20("$CROPORN", "CRP") {

    	dividendTracker = new CropornDividendTracker();
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x085d8C985C73Fbb425398209d864Addd062fF3c1);         
        // for testnet cron: 0x085d8C985C73Fbb425398209d864Addd062fF3c1
        ...
        maxTxAmount = totalSupply().div(1000); // 0.1%. 
        maxTokensPerAddress = totalSupply().mul(3).div(1000); //0.3%
    }
    ...
    function setTxLimitExempt(address account, bool excluded) external onlyOwner {
        require(isTxLimitExempt[account] != excluded, "CROPORN::setTxLimitExempt: Account is already the value of 'excluded'");
    ...
    function setExcludedFromAntiWhale(address account, bool excluded) external onlyOwner {
        require(isExcludedFromAntiWhale[account] != excluded, "CROPORN::setExcludedFromAntiWhale: Account is already the value of 'excluded'");
    ...
    function setExcludedFromTransactionCooldown(address account, bool excluded) public onlyOwner {
        require(isExcludedFromTransactionlock[account] != excluded, "CROPORN::setExcludedFromTransactionCooldown: Account is already the value of 'excluded'");
    ...
    function setTransactionCooldownTime(uint256 transactiontime) public onlyOwner {
        require(transactiontime <= 86400, "CROPORN::setTransactionCooldownTime: Can not to exceed max amount");
        emit TransactionCooldownTimeUpdated(transactionLockTime, transactiontime);
        transactionLockTime = transactiontime;
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CROPORN::excludeFromFees: Account is already the value of 'excluded'");
    ...
    function setBlacklist(address account, bool excluded) public onlyOwner {
        require(_isBlacklisted[account] != excluded, "CROPORN::setBlacklist: Account is already the value of 'excluded'");
    ...
    function enableTrading(bool value) external onlyOwner {
        require(!trading, "CROPORN::setTrading: Trading is already set.");
        trading = value;
    }

    function enableImpactEffect(bool value) external onlyOwner {
        require(value != impactEnabled, "CROPORN::enableTrading: ImpactEnabled is already set.");
        impactEnabled = value;
    }

    function setMarketingWallet(address payable wallet) external onlyOwner{
        require(wallet != address(0), "CROPORN::setMarketingWallet: Please use vaild payable address.");
    ...
    function setAlgorithmWallet(address payable wallet) external onlyOwner{
        require(wallet != address(0), "CROPORN::setAlgorithmWallet: Please use vaild payable address.");
    ...
    function setBuybackWallet(address payable wallet) external onlyOwner{
        require(wallet != address(0), "CROPORN::setBuybackWallet: Please use vaild payable address.");
        emit BuybackWalletUpdated(msg.sender, _buybackWalletAddress, wallet);
        _buybackWalletAddress = wallet;
    }

    function setLiquidityFee(uint256 value) external onlyOwner{
        totalFees = value.add(WCRORewardsFee).add(marketingFee).add(burnFee).add(buybackFee).add(algorithmFee);
        require(totalFees <= 30, "CROPORN::setLiquidityFee: Too high Fees");
        emit LiquidityFeeUpdated(msg.sender, liquidityFee, value);
        liquidityFee = value;
    }

    function setWCRORewardsFee(uint256 value) external onlyOwner{
        totalFees = value.add(liquidityFee).add(marketingFee).add(burnFee).add(buybackFee).add(algorithmFee);
        require(totalFees <= 30, "CROPORN::setWCRORewardsFee: Too high Fees");
        emit WCRORewardsFeeUpdated(msg.sender, WCRORewardsFee, value);
        WCRORewardsFee = value;
    }

    function setMarketingFee(uint256 value) external onlyOwner{
        totalFees = value.add(WCRORewardsFee).add(liquidityFee).add(burnFee).add(buybackFee).add(algorithmFee);
        require(totalFees <= 30, "CROPORN::setMarketingFee: Too high Fees");
        emit MarketingFeeUpdated(msg.sender, marketingFee, value);
        marketingFee = value;
    }

    function setBurnFee(uint256 value) external onlyOwner{
        totalFees = value.add(WCRORewardsFee).add(marketingFee).add(liquidityFee).add(buybackFee).add(algorithmFee);
        require(totalFees <= 30, "CROPORN::setBurnFee: Too high Fees");
        emit BurnFeeUpdated(msg.sender, burnFee, value);
        burnFee = value;
    }

    function setBuybackFee(uint256 value) external onlyOwner{
        totalFees = value.add(WCRORewardsFee).add(marketingFee).add(burnFee).add(liquidityFee).add(algorithmFee);
        require(totalFees <= 30, "CROPORN::setBuybackFee: Too high Fees");
        emit BuybackFeeUpdated(msg.sender, buybackFee, value);
        buybackFee = value;
    }

    function setAlogrithmFee(uint256 value) external onlyOwner{
        totalFees = value.add(WCRORewardsFee).add(marketingFee).add(burnFee).add(buybackFee).add(liquidityFee);
        require(totalFees <= 30, "CROPORN::setAlogrithmFee: Too high Fees");
        emit AlgorithmFeeUpdated(msg.sender, algorithmFee, value);
        algorithmFee = value;
    }

    function setSellFee(uint256 value) external onlyOwner{
        require(sellFee <= 10, "CROPORN::setSellFee: Too high Fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CROPORN::setAutomatedMarketMakerPair: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CROPORN::_setAutomatedMarketMakerPair: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CROPORN::updateGasForProcessing: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CROPORN::updateGasForProcessing: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(from != address(0), "CROPORN: transfer from the zero address");
        require(to != address(0), "CROPORN: transfer to the zero address");
        require(amount <= maxTxAmount || isTxLimitExempt[from], "CROPORN: transfer amount limit exceeded");
        require(!_isBlacklisted[to] && !_isBlacklisted[from], 'CROPORN: Blacklisted address');
        require(antiWhaledisabled || isExcludedFromAntiWhale[to] || balanceOf(to) + amount <= maxTokensPerAddress, "CROPORN: Max tokens limit for this account exceeded. Or try lower amount");
        ...
        if( !trading && 
            ...
            require( !automatedMarketMakerPairs[to], "CROPORN: Trading disabled");
            require( !automatedMarketMakerPairs[from], "CROPORN: Trading disabled");
        }
        ...
        if( swapAndLiquifyDividendEnabled &&
            ...
            //swapAndSendToFeeWithCro(marketingTokens, _marketingWalletAddress);
            ...
            //swapAndSendDividendsWithCro(sellTokens);
    ...
    function swapAndSendToFee(uint256 tokens, address to) private  {
        uint256 initialWCROBalance = IERC20(WCRO).balanceOf(address(this));

        swapTokensForWCRO(tokens);
        uint256 newBalance = IERC20(WCRO).balanceOf(address(this)).sub(initialWCROBalance);
        IERC20(WCRO).transfer(to, newBalance);
    }

    function swapAndSendToFeeWithCro(uint256 tokens, address to) private  {
        // Cro amount 
        ...
        (bool success, ) = to.call{value:newBalance}("");    
        require(success, "CROPORN::swapAndSendToFeeWithCro: CRO transfer reverted because of insufficient CRO balance");
    }

    /*
    function swapAndSendDividendsWithCro(uint256 tokens) private {
        // Cro amount 
        uint256 initialBalance = address(this).balance;
        ...
        // swapped Cro amount
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 dividends = newBalance;
        (bool success, ) = address(dividendTracker).call{value:dividends}("");    
        require(success, "CROPORN::swapAndSendDividendsWithCro: CRO transfer reverted because of insufficient CRO balance");
        dividendTracker.distributeWCRODividends(dividends);
    ...
    function swapTokensForWCRO(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private{
        swapTokensForWCRO(tokens);
        uint256 dividends = IERC20(WCRO).balanceOf(address(this));
        try IERC20(WCRO).transfer(address(dividendTracker), dividends){
            dividendTracker.distributeWCRODividends(dividends);
            emit SendDividends(tokens, dividends);
        }
        catch {
            revert("CROPORN::WCRO transfer reverted because of insufficient WCRO balance");
        }
    }    
    ...
    function _priceImpactTax(uint256 amount) internal view returns (bool) {
        ...
        if (_reserveA == 0 && _reserveB == 0) {
            ...
            uint256 receivedWCRO = _reserveB_new.sub(_reserveB);

            uint256 _new_price = (amount.div(receivedWCRO)).mul(10**18);
    ...
    function withdrawCroporn(address toAddress, uint256 amount) external onlyOwner {
        uint256 maxblance = balanceOf(address(this));
        if(maxblance <= amount) {
            amount = maxblance;
        }
        super._transfer(address(this), toAddress, amount);
        emit CropornTokensWithdrawn(msg.sender, toAddress, amount);
    }
...
contract CropornDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("CROPORN_Dividen_Tracker", "CROPORN_DTker") {
    	claimWait = 21600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        revert("Cropron_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        revert("Cropron_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CROPORN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Cropron_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Cropron_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }

    function updateMinimumTokenBalanceForDividends(uint256 newMinimumTokenBalanceForDividends) external onlyOwner {
        require(newMinimumTokenBalanceForDividends >= 100 * (10**18) && newMinimumTokenBalanceForDividends <= 10000000 * (10**18), "Cropron_Dividend_Tracker: MinimumTokenBalanceForDividends must be updated to between 100 and 10000000");
        require(newMinimumTokenBalanceForDividends != minimumTokenBalanceForDividends, "Cropron_Dividend_Tracker: Cannot update MinimumTokenBalanceForDividends to same value");
